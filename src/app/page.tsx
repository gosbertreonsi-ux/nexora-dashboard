...
export default function Dashboard() {
  const [balanceMetricUnits, setBalanceMetricUnits] = useState<string>("500000");
  ...
  // Seed initial values to avoid rendering empty boxes
  ...
  useEffect(() => {
    ...
    socket.onopen = () => {
      setIsConnected(true);
      console.log("🟢 Connected to Gravity stream mesh channel");
    };

    socket.onmessage = (event) => {
      try {
        const data = JSON.parse(event.data);
        console.log("📬 Log data token captured over edge websocket:", data);
        
        if (data.status === "SUCCESS") {
          setTotalChecks((prev) => {
            const nextCount = prev + 1;
            setChartData((curr) => [...curr.slice(1), Math.min((nextCount * 8) % 100, 100)]);
            return nextCount;
          });
          setBalanceMetricUnits(data.balanceMetricUnits || "500000");
    ...
    socket.onclose = () => {
      ...
      console.log("🔴 Socket connection split from infrastructure channel");
    };
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-slate-50 p-8 font-sans">
      ...
      <section className="bg-slate-900 border border-slate-800 rounded-xl p-6 mb-8">
        ...
        <div className="h-28 flex items-end gap-2 pt-4 px-2 border-b border-slate-800 border-l border-slate-800">
          {chartData.map((val, idx) => (
            <div 
              key={idx} 
              style={{ height: `${Math.max(val, 8)}%` }} 
              className="flex-1 bg-gradient-to-t from-emerald-600 to-emerald-400 rounded-t transition-all duration-300 shadow-[0_0_10px_rgba(52,211,153,0.1)]"
            />
          ))}
        </div>
      </section>

      {/* INGESTION TABLE LOGS SECTION */}
          ...
          <table className="w-full text-left font-mono text-xs border-collapse">
            <thead>
              <tr className="border-b border-slate-800 text-slate-400">
                ...
                <th className="pb-3 font-medium">Attestation Status</th>
                <th className="pb-3 font-medium">Micro-Fee Entry</th>
                <th className="pb-3 font-medium text-center">Security Override</th>
            ...
            <tbody>
                  ...
                  <td className="py-3 text-center">
                    {log.status === 'PASS' ? (
                      <button 
                        ...
                        className="bg-rose-950/40 text-rose-400 border border-rose-900/60 px-3 py-1 rounded hover:bg-rose-900 hover:text-white transition-all duration-200 cursor-pointer font-sans text-[11px]"
                      >
                        Isolate & Ban
                      </button>
                    ) : (
              ...
              {liveLogs.length === 0 && (
                <tr>
                  <td colSpan={6} className="py-8 text-center text-slate-500 italic">Awaiting device proofs-of-presence network packets... (Ensure test-client.ts is running)</td>
                </tr>
              )}
            </tbody>
          </table>
